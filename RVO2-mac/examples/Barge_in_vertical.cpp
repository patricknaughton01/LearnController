...
std::vector<std::vector<RVO::Vector2> > setupScenario(RVO::RVOSimulator *sim)
{
    ...
    if (VERTICAL_WALL) {
        ...
        RVO::Vector2 robot_pos(randomize(-0.15f + wall_width + wall_dist / 2.0, wall_width + wall_dist / 2.0 + 0.15f), 
                            randomize(1.0f, 1.5f));
        ...
        RVO::Vector2 human_pos1(randomize(wall_width + 0.1f, wall_width + wall_dist / NUM_PEOPLE - 0.1f), 
                                randomize(wall_length + 0.2f, wall_length + 0.4f));
        RVO::Vector2 human_goal1(human_pos1.x() - randomize(0.3f, 0.5f),
                                 human_pos1.y() + randomize(1.0f, 2.0f));    

        RVO::Vector2 human_pos2(randomize(wall_width + wall_dist / NUM_PEOPLE + 0.1f, wall_width + wall_dist / NUM_PEOPLE * 2 - 0.1f), 
                                randomize(wall_length + 0.2f, wall_length + 0.4f));
        RVO::Vector2 human_goal2(human_pos2.x() - randomize(0.4f, 0.8f),
                                 human_pos2.y() + randomize(1.0f, 2.0f));    

        RVO::Vector2 human_pos3(randomize(wall_width + wall_dist / NUM_PEOPLE * 2 + 0.1f, wall_width + wall_dist - 0.1f), 
                                randomize(wall_length + 0.2f, wall_length + 0.4f));
        RVO::Vector2 human_goal3(human_pos3.x() + randomize(0.4f, 0.8f),
                                 human_pos3.y() + randomize(1.0f, 2.0f));    
        
        RVO::Vector2 human_pos4(randomize(wall_width + wall_dist / NUM_PEOPLE * 3 + 0.1f, wall_width + wall_dist - 0.1f), 
                                randomize(wall_length + 0.2f, wall_length + 0.4f));

        RVO::Vector2 human_goal4(human_pos4.x() + randomize(0.3f, 0.5f),
                                 human_pos4.y() + randomize(1.0f, 2.0f));    
        ...
        RVO::Vector2 robot_pos(randomize(-0.5f, 0.5f),
        randomize(-0.5f + wall_width + wall_dist / 2.0, wall_width + wall_dist / 2.0 + 0.5f));
        RVO::Vector2 robot_goal(randomize(-0.5f + wall_width + wall_dist / 2.0, wall_width + wall_dist / 2.0 + 0.5f), 
                            randomize(4.5f, 5.5f));

        RVO::Vector2 human_pos1(randomize(wall_width, wall_width + wall_dist / 2.0), 
                                randomize(wall_length + 0.5f, wall_length + 1.0f));
        RVO::Vector2 human_goal1(randomize(wall_width / 2.0 - 0.5f, wall_width / 2.0 + 0.5f), 
                                randomize(wall_length + 1.5f, wall_length + 2.5f));


        RVO::Vector2 human_pos2(randomize(wall_width + wall_dist / 2.0, wall_width + wall_dist), 
                                randomize(wall_length + 0.5f, wall_length + 1.0f));
        RVO::Vector2 human_goal2(randomize(wall_width + wall_dist + wall_width / 2.0, wall_width + wall_dist + wall_width / 2.0 + 1.0f), 
                                randomize(wall_length + 1.5f, wall_length + 2.5f));   
        
        sim->addAgent(robot_pos, 1.0f, 10, 5.0f, 5.0f, randomize(0.15f, 0.25f), randomize(1.5f, 2.0f));
        goals.push_back(robot_goal);
        
        sim->addAgent(human_pos1, 1.0f, 10, 5.0f, 5.0f, randomize(0.15f, 0.25f), randomize(0.1f, 0.5f));
        goals.push_back(human_goal1);

        sim->addAgent(human_pos2, 1.0f, 10, 5.0f, 5.0f, randomize(0.15f, 0.25f), randomize(0.1f, 0.5f));
        goals.push_back(human_goal2);
    }
