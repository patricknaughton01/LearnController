...
def dataloader(data, config, is_train=True, shuffle=True):
    ...
    # print('unique num of humans: ', np.unique(num_humans))
    for num_human in np.unique(num_humans):
        # print('processing number of humans: %d' % num_human)
        filt_indices = num_humans == num_human 
        filt_states = states[filt_indices]
        filt_seq_lengths = seq_lengths[filt_indices]
