...
def main():
    ...
    for c in [0.9 + j/100.0 for j in range(9, 10)]:
        failures = 0
        for i in tqdm(range(args.num_episodes)):
            ...
            sim = Simulator(scene=scene, reverse=args.reverse,
                            single=args.single, file=out_file)
            success_model = None
            reverse_model = None
        ...
        if args.record:
                out_file.close()
        print("Failed {} times out of {} runs with a conf of {}".format(
            failures, args.num_episodes, c))
    reward_file.close()
