...
class Controller(nn.Module):
    def __init__(self, config, model_type='crossing'):
        ...
        attention_dims = [int(x) for x in
                          config.get(model_type, 'attention_dims').split(
                              ', ')]  # 64, 32, 1
        if self.with_global_state:
            self.attention = mlp(mlp1_dims[-1] * 2,
    ...
    def forward(self, state):
        ...
        #batch_state = self_state.repeat(size[1], 1)     # size[1] is batch
        # size
